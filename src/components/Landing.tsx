...
function Landing() {
    ...
    useLayoutEffect(() => {
        let ctx = gsap.context(() => {
            gsap.fromTo(".main-text", { y: "+=360", opacity: 0}, {y: "0", opacity: 1, duration: .5, ease: "ease-in"});
    ...
    return (
        <div className="flex items-center justify-center h-screen w-screen bg-landing bg-center bg-cover from-indigo-500 backdrop-blur-md" ref={comp}>
            ...
            <img src={logo} className="logo w-1/3 h-1/3 tablet:hidden desktop:block" />
        </div>
    )
}
